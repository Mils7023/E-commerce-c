...
export const BestSellingProducts = () => {
  const { bestSellingToppers } = useContext(HomeContext);
  ...
  return (
    <section className={`overflow-hidden`} id="bestselling_products_section">
          ...
          <div>
            <div className="main-title d-flex align-items-center justify-content-between">
              <h2 className="h2 black_text jua-font">
                Our Best
                <br className="d-sm-none d-block" /> Selling Toppers
              </h2>
              <Link
            ...
            <div
              className={`${styles.bestselling_products_row} p_cards_row scrollbar row flex-lg-wrap scrollbar  flex-nowrap g-xxl-4 g-3`}
            >
              {bestSellingToppers.error && (
              ...
              {!bestSellingToppers.data?.items
                ? [0, 1, 2, 3, 4, 5, 6, 7].map((index) => (
                    <div
                      key={index}
                      className={`${styles.bestselling_products_col} p_cards_col col-xl-3 col-lg-4 col-md  col-sm-6 col`}
                    >
                      <ProductCardSkeleton />
                    </div>
                  ))
                : bestSellingToppers.data?.items &&
                  bestSellingToppers?.data?.items?.map((product: Product) => (
                    <div
                      key={product.id}
                      className={`${styles.bestselling_products_col} p_cards_col col-xl-3 col-lg-4 col-md  col-sm-6 col`}
                    >
