...
import styles from "./SeasonalProducts.module.scss";
...
export const SeasonalProducts = () => {
  const { seasonalToppers } = useContext(HomeContext);
        ...
        <div className="container">
          <div>
            <div className="main-title d-flex align-items-center justify-content-between">
              <h2 className="h2 black_text jua-font">Seasonal Toppers</h2>
              <Link
                href=""
            ...
            <div
              className={`${styles.seasonal_products_row} p_cards_row scrollbar row flex-lg-wrap scrollbar  flex-nowrap g-xxl-4 g-3`}
            >
              {seasonalToppers.error && (
              ...
              {!seasonalToppers.data?.items
                ? [0, 1, 2, 3, 4, 5, 6, 7].map((index) => (
                    <div
                      key={index}
                      className={`${styles.seasonal_products_col} p_cards_col col-xl-3 col-lg-4 col-md  col-sm-6 col`}
                    >
                      <ProductCardSkeleton />
                    </div>
                  ))
                : seasonalToppers.data?.items &&
                  seasonalToppers?.data?.items?.map((product: Product) => (
                    <div
                      key={product.id}
                      className={`${styles.seasonal_products_col} p_cards_col col-xl-3 col-lg-4 col-md  col-sm-6 col`}
                    >
